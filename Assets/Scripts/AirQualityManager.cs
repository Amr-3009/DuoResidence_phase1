...
public class AirQualityManager : MonoBehaviour
{
    ...
    [Tooltip("How often (in seconds) the simulation sends network updates. 0.1s = 10 updates per second. Prevents broker choking.")]
    ...
    void Start() //
    {
        ...
        // Send out baseline state immediately on boot
    ...
    void Update() //
    {
        ...
        // 2. Continuous local high-speed gas decay simulation calculations
        ...
        // 3. Keep local UI sliders perfectly responsive
        ...
        // 4. FIXED: Throttled network tick window logic loop
    ...
    private void ForceNetworkBroadcast()
    {
        ...
        // Broadcast to physical local fan rotators
    ...
    private void PublishJsonTelemetry(float currentPercentage) //
    {
        ...
        {
            // FIX: retain parameter turned to TRUE to solve Project B's startup values!
    ...
    private string GetAirQualityStatusString(int severityLevel) { switch (severityLevel) { case 0: return "<color=green>Excellent</color>"; case 1: return "<color=yellow>Warning</color>"; case 2: return "<color=red>CRITICAL</color>"; default: return "Unknown"; } } //
